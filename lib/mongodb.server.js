...
export async function getDb(shopName = null) {
  if (!clientPromise) {
    throw new Error("MongoDB connection not initialized. Check your MONGODB_URI.")
  }
...
// Store and retrieve shop name for reuse
...
export async function connectToDatabase(shopName = null) {
  try {
    ...
    return { client, db, dbName, isConnected };
  } catch (error) {
...
// Check database connection with specific shop
export async function checkDatabaseConnection(shopName = null) {
  try {
    const { client, dbName } = await connectToDatabase(shopName);
...
export async function getActiveCampaign(shopName) {
  // Placeholder for fetching active campaign from MongoDB
  console.log(`Fetching active campaign for shop: ${shopName}`);
  return {
    id: "fake-campaign-id",
    name: "Demo Campaign",
    primaryColor: "#ffc700",
    ...
    content: {
      landing: {
        title: "TRY YOUR LUCK",
        subtitle: "This is a demo of our Spin to Win",
        buttonText: "SPIN",
        emailPlaceholder: "Enter your Email",
        privacyPolicyText: "I accept the terms and conditions",
      },
      result: {
